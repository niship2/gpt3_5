    ...
    with st.container():
        with abst_inst_col:
            instruction_abst = st.text_area(
                '要約生成指示文を入力してください', value="特許文章らしい要約として修正してください。【課題】と【解決手段】という見出しを加えてください。であるという語尾で作成して下さい。", placeholder="特許文章らしい要約を作成してください。【課題】と【解決手段】という見出しを加えてください。であるという語尾で作成して下さい。")
            if st.button("要約修正！"):
    ...
    with st.container():
        with claims_inst_col:
            instruction_claim_opt = st.multiselect(
                ...
                options=('特許文章らしい特許請求の範囲で記載してください',
                         '各特許請求の範囲は１文で作成して下さい',
                         '【請求項１】という見出しを加えて下さい',
                         '【請求項2】を追加して、請求項2には請求項１の構成のどれかを具体化したものとして生成してください')

            )
            instruction_claims_text = "。\n".join(merge_instruction(
                instruction_claim_opt, instruction_claims_text)) + "。"  # 最後の分に。を付加
            instruction_claims = st.text_area(
                '請求項生成指示文を作成してください', value=instruction_claims_text, placeholder="", height=250)
    ...
    with st.container():
        with desc_inst_col:
            instruction_desc_opt = st.multiselect(
                ...
                options=('特許文章らしい明細書の文章として修正してください',
            ...
            instruction_desc_text = "。\n".join(merge_instruction(
                instruction_desc_opt, instruction_desc_text)) + "。"  # 最後の分に。を付加
            instruction_desc = st.text_area(
                '明細書生成指示文を入力してください', value=instruction_desc_text, placeholder="", height=250)
            if st.button("明細書修正！"):
